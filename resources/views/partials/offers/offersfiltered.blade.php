            ...
            <div class="col-lg-12">
                ...
                <div class="offers_grid">
                    ...
                    @foreach($offers as $offer)
                        <div class="offers_item rating_{{$offer->stars}}">
                            <div class="row">
                                <div class="col-lg-1 temp_col"></div>
                                <div class="col-lg-3 col-1680-4">
                                    <div class="offers_image_container">
                                        <!-- Image by https://unsplash.com/@kensuarez -->
                                        <div class="offers_image_background" style='background-image:url({{asset("$offer->img_hotel_path")}})'></div>
                                        <div class="offer_name"><a href="{{route('single',['id'=>$offer->ahid])}}">{{$offer->name}}</a></div>
                                    </div>
                                </div>
                                <div class="col-lg-8">
                                    <div class="offers_content">
                                        <div class="offers_price">${{$offer->avg_hotel_price}}<span>per night</span></div>
                                        @if($offer->stars==5)
                                        ...
                                        <p class="offers_text">{{$offer->short_desc}}</p>
                                        <div class="offers_icons">
                                            <ul class="offers_icons_list">
                                                @if($offer->wireless)
                                                    <li class="offers_icons_item"><img src="{{asset('images/wifi-2.png')}}" alt="wifi"></li>
                                                @endif
                                                @if($offer->pool)
                                                    <li class="offers_icons_item"><img src="{{asset('images/pool.png')}}" alt="pool"></li>
                                                @endif
                                                @if($offer->wheelchair)
                                                    <li class="offers_icons_item"><img src="{{asset('images/wheel.png')}}" alt="invalid"></li>
                                                @endif
                                                @if($offer->smoking_area)
                                                    <li class="offers_icons_item"><img src="{{asset('images/smoke.png')}}" alt="smoke"></li>
                                                @endif
                                            </ul>
                                        </div>  <?php $idU = session()->get('user')[0]->aid ?>
                                        @if(session()->get('user'))
                                            <div class="button book_button trans_200"><a href="/book/<?=$idU?>/{{$offer->ahid}}">book<span></span><span></span><span></span></a></div>
                                        @else
                                            <div class="button book_button trans_200"><a href="/login">book<span></span><span></span><span></span></a></div>
                                        @endif
                                        <div class="offer_reviews">
                                            <div class="offer_reviews_content">
                                                @if(number_format($offer->prosekOcena,1)>5)
                                                    <div class="offer_reviews_title">very good</div>
                                                @elseif(number_format($offer->prosekOcena,1)>3)
                                                    <div class="offer_reviews_title">good</div>
                                                @else
                                                    <div class="offer_reviews_title">bad</div>
                                                @endif
                                                <div class="offer_reviews_subtitle">{{$offer->brojOcena}} reviews</div>
                                            </div>
                                            <div class="offer_reviews_rating text-center">{{number_format($offer->prosekOcena,1)}}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
